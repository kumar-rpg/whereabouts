...
export default function MyActivityDetailPage({ params }: PageProps<'/my/[id]'>) {
  ...
  const { data, isLoading, error } = useSWR(`my-activity-${id}`, () => fetchActivity(id))
  ...
  if (error || !data) return (
    <div className="page-container">
      <div className="empty-state">
        ...
        <Link href="/my" className="btn btn-outline" style={{ marginTop: 8 }}>← My Activities</Link>
      </div>
    </div>
  )
    ...
    <div className="page-container">
      ...
      {editing ? (
        <div>
          ...
          <ActivityForm initial={data} returnPath={`/my/${id}`} />
        </div>
      ) : (
        <>
          {/* Header */}
